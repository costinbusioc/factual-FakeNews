...
for link_no in range(1, MAX_PAGE_NO):
	link = root_link + "page=" + str(link_no) + "&category=coronavirus"
	...
	for row in rows:
		...
		category = content.findNext('picture').find('img')['alt'].strip()
...
write_csv('datasetCoronavirus.csv', cols_name, cols)
